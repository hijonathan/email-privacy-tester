...
#     client.query 'DELETE FROM email WHERE ctime < UNIX_TIMESTAMP(NOW())*1000 - ?', [ conf.expireDataAfter * 1000 ], ( err, info ) ->
...
exports.submit = ( req, res ) ->
  email = ept_email.emailToUnicode req.body?.email or ''
  email = email.replace /[\r\n]+/g, ''
    ...
    optedOut email, ( status ) ->
      ...
      if ept_email.validateFormat email
        ...
        pg.connect process.env.DATABASE_URL, (err, client) ->
          ...
          client.query sql, [ Date.now(), clientIP(req), salt, salt, removePlusAddressing email ], ( err, info ) ->
            if err?
...
clientIP = ( req ) ->
  ...
  if client_ip is '127.0.0.1' then client_ip = (req.get('x-forwarded-for') or '').replace /.*,\s*/, ''
  return client_ip
...
optedOut = ( email, cb ) ->
  ...
  pg.connect process.env.DATABASE_URL, (err, client) ->
    ...
    client.query sql, [ ept_email.emailToUnicode removePlusAddressing email ], ( err, info ) ->
