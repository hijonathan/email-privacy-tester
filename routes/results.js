...
(function() {
  var conf, dbh, emitter, getNewData, getNewEmailStatus, getNewTestHits, getResults, humanAge, minHumanAge, mysql, sendHTML, sendJSON, testCallbackDNS, testCallbackEmail, testCallbackHTTP, tests, util;
  ...
  testCallbackHTTP = function(req, res, email, callbackCode, name, clientIP) {
    ...
    return dbh.query(sql, [email.email_id, Date.now(), name, clientIP, req.get('user-agent'), httpXForwardedFor], function(err, info) {
      if (err == null) {
  ...
  testCallbackEmail = function(req, res, email, callbackCode, name) {
    ...
    return dbh.query(sql, [email.email_id, ctime, name, subject], function(err, info) {
      if (err == null) {
        return emitter.emit("newCallback" + email.lookupCode);
      }
    });
  ...
  testCallbackDNS = function(req, res, email, callbackCode, name) {
    ...
    return dbh.query(sql, [email.email_id, ctime, name, clientIP], function(err, info) {
      if (err == null) {
        return emitter.emit("newCallback" + email.lookupCode);
      }
    });
  ...
  exports.emailStatusCallback = function(req, res) {
    ...
    return getResults('callback_code', callbackCode, function(err, email) {
      ...
      return dbh.query(sql, [status, email.email_id, mtime, message], function(err, info) {
        if (err == null) {
  ...
  getResults = function(lookupType, code, callback) {
    return dbh.query("SELECT email_id, lookup_code AS lookupCode, ctime FROM email WHERE " + lookupType + "=?", [code], function(err, info) {
      if (err != null) {
        callback(err);
      }
      if (info.length) {
  ...
  getNewEmailStatus = function(lookupCode, emailLogId, callback) {
    return dbh.query('SELECT email_log.email_log_id AS emailLogId, email_log_type.name AS status, email_log.ctime AS mtime, email_log.message FROM email LEFT JOIN email_log ON email.email_id=email_log.email_id LEFT JOIN email_log_type ON email_log.email_log_type_id=email_log_type.email_log_type_id WHERE email.lookup_code=? AND email_log.email_log_id > ? ORDER BY email_log.email_log_id', [lookupCode, emailLogId], function(err, rows) {
  ...
  getNewTestHits = function(lookupCode, callbackId, callback) {
    ...
    return dbh.query(sql, [lookupCode, callbackId], function(err, rows) {
      var hit, name, results, _i, _len;
